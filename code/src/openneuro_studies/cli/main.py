...
def metadata_generate(
    ...
    # Generate combined derivative hierarchical TSV files for counts/sizes stages
    if stage in ("counts", "sizes", "imaging"):
        from bids_studies.extraction.study import extract_all_derivatives_stats

        click.echo(
            f"\nGenerating per-study derivative hierarchical statistics "
            f"({'parallel' if jobs > 1 else 'sequential'}, {jobs} job{'s' if jobs > 1 else ''})..."
        )

        def process_study_derivatives(study_path: Path, source_id: str) -> tuple[Path, bool, str | None]:
            """Process all derivatives for a study, returning (study_path, success, error_msg)."""
            import logging
            logger = logging.getLogger(__name__)

            derivatives_dir = study_path / "derivatives"
            try:
                logger.info(f"Processing derivative stats for {study_path.name}")
                extract_all_derivatives_stats(
                    derivatives_dir, source_id=source_id, write_files=True
                )
                logger.info(f"  ✓ {study_path.name} derivative extraction complete")
                return study_path, True, None
            except Exception as e:
                logger.error(f"  ✗ {study_path.name} derivatives failed: {e}", exc_info=True)
                return study_path, False, str(e)

        # Collect studies that have derivatives
        deriv_study_tasks = []
        for study_path in study_paths:
            derivatives_dir = study_path / "derivatives"
            ...
            deriv_study_tasks.append((study_path, source_id))

        if deriv_study_tasks:
            if jobs > 1:
                with ThreadPoolExecutor(max_workers=jobs) as executor:
                    futures = {executor.submit(process_study_derivatives, sp, sid): sp for sp, sid in deriv_study_tasks}
                    for future in as_completed(futures):
                        study_path, success, error = future.result()
                        derivatives_dir = study_path / "derivatives"
                        if success:
                            click.echo(f"  ✓ {study_path.name}/derivatives/derivatives+*.tsv")
                            for tsv in derivatives_dir.glob("derivatives+*.tsv"):
                                modified_paths.append(tsv)
                            for json_file in derivatives_dir.glob("derivatives+*.json"):
                                modified_paths.append(json_file)
                        else:
                            click.echo(f"  ✗ {study_path.name}: {error}", err=True)
            else:
                for study_path, source_id in deriv_study_tasks:
                    study_path, success, error = process_study_derivatives(study_path, source_id)
                    derivatives_dir = study_path / "derivatives"
                    if success:
                        click.echo(f"  ✓ {study_path.name}/derivatives/derivatives+*.tsv")
                        for tsv in derivatives_dir.glob("derivatives+*.tsv"):
