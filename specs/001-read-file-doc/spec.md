...
- **FR-009**: System MUST generate studies.tsv with study_id, name, version, raw_version, bids_version, hed_version, license, authors, author_lead_raw, author_senior_raw, source_count, source_types, derivative_count, subjects_num, sessions_num, sessions_min, sessions_max, bold_num, t1w_num, t2w_num, bold_size, t1w_size, bold_size_max, bold_duration_total, bold_duration_mean, bold_voxels_total, bold_voxels_mean, datatypes, derivative_ids, and bids_valid columns. Column definitions: source_count is number of sourcedata subdatasets; source_types is comma-separated set of BIDS DatasetTypes from source datasets (e.g., "raw", "derivative", "raw,derivative"); derivative_count is number of derivative subdatasets; authors are from git shortlog of study dataset; author_lead_raw and author_senior_raw are first/last elements from Authors in raw dataset_description.json, using "n/a" if multiple sources with conflicting values, or duplicating if single author; bold_duration_total is sum of TR×volumes across all BOLD runs in seconds; bold_duration_mean is weighted mean BOLD run duration; bold_voxels_total is total voxels across all BOLD files; bold_voxels_mean is mean voxels per BOLD weighted by duration
...
- **FR-042b**: System MUST generate `sourcedata+datasets.tsv` within each study's sourcedata/ directory containing per-source-dataset aggregated statistics. Columns include subjects_num, sessions_num, sessions_min, sessions_max, bold_num, t1w_num, t2w_num, bold_size, t1w_size, bold_size_max, bold_duration_total, bold_duration_mean, bold_voxels_total, bold_voxels_mean, datatypes.
- **FR-042c**: System MUST generate corresponding JSON sidecar files (sourcedata+subjects.json, sourcedata+datasets.json) describing column purposes and units following BIDS conventions.
...
- **FR-042f**: System MUST aggregate stats from `sourcedata+datasets.tsv` and `derivatives+datasets.tsv` to populate corresponding columns in studies.tsv. Aggregation uses sum for counts and totals, weighted mean for averages (weighted by bold_num for duration, by duration for voxels).
