...
- **FR-042**: System MUST support hierarchical stats extraction at multiple levels: per-subject (within each source/derivative dataset), per-dataset (aggregated from subjects), per-study (aggregated from datasets), and across studies (aggregated in studies.tsv). Stats are extracted at the lowest level and consolidated upward. **Status**: Design approved (see doc/designs/20251226-hierarchical-stats-extraction.md), implementation pending. Current implementation provides only study-level extraction (studies.tsv).
...
- **FR-042c**: System MUST generate corresponding JSON sidecar files (sourcedata+subjects.json, sourcedata.json) describing column purposes and units following BIDS conventions.
...
- **FR-042e**: System MUST generate `derivatives+subjects.tsv` and `derivatives+datasets.tsv` within each study's derivatives/ directory for derivative-specific statistics (output_num, output_size, nifti_num, nifti_size, html_num).
- **FR-042f**: System MUST aggregate stats from `sourcedata.tsv` and `derivatives+datasets.tsv` to populate corresponding columns in studies.tsv. Aggregation uses sum for counts and totals, max for spatial dimensions (bold_voxels), and dict merge with count summation for distributions (bold_trs). No weighted means are computed; TR distributions preserve heterogeneity across acquisitions.
