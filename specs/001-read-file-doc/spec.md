...
6. **Given** `--test-filter` is used with `--include-related derivatives`, **When** the discovery script runs, **Then** the filter is automatically expanded to include all derivatives whose source_datasets intersect with the filtered set, recursively (so derivatives of derivatives are also included)
7. **Given** `--test-filter` is used with `--include-related all`, **When** the discovery script runs, **Then** the filter is expanded bidirectionally: forward to derivatives (as in scenario 6) AND backward from derivatives to their source datasets, iterating until transitive closure is reached

---

### User Story 2 - Metadata Generation and Synchronization (Priority: P2)

As a dataset curator, I need automatically generated and synchronized metadata files (dataset_description.json, studies.tsv, studies+derivatives.tsv) for each study and the overall collection, so that I can track dataset provenance, licensing, versioning, and available derivatives without manual maintenance.
...
**Independent Test**: Can be tested by verifying that running the metadata generation script on an existing study produces correct and reproducible dataset_description.json with BIDS 1.10.1 study format, populates studies.tsv with accurate summary data, and creates studies+derivatives.tsv listing all available derivatives with versions.

**Acceptance Scenarios**:

1. **Given** a study folder with sourcedata/{dataset_id}/ (consistent naming per FR-003d), **When** metadata generation runs, **Then** dataset_description.json is created with DatasetType="study", Authors from git shortlog of the study dataset, Title prefixed with "Study dataset for ", and SourceDatasets referencing all sourcedata entries
2. **Given** multiple studies with varying metadata, **When** studies.tsv generation runs, **Then** the file contains study_id (e.g. study-ds000001), name, version, raw_version, bids_version, hed_version, license, authors (from git shortlog of study dataset), author_lead_raw (first element from Authors in raw dataset_description.json), author_senior_raw (last element from Authors in raw dataset_description.json), subjects_num, sessions_num, sessions_min, sessions_max, bold_num, t1w_num, t2w_num, bold_size, t1w_size, bold_size_max, bold_voxels, datatypes, derivative_ids (list of derivative identifiers), and bids_valid (where version is study dataset version, raw_version is version/tag of raw dataset if single source and released or "n/a" if multiple sources or no release; for author_lead_raw and author_senior_raw: if multiple sources with all same values use that value, else "n/a"; if single author in raw dataset duplicate for both lead and senior)
...
3. **Given** a study with 3 derivatives (e.g., fmriprep-21.0.1, mriqc-23.0.0, bids-validator), **When** studies+derivatives.tsv generation runs, **Then** 3 rows are created with study_id, derivative_id pairs, each listing tool name, version, size statistics from git annex info, execution metrics if available, and outdatedness (number of commits the processed raw dataset is behind current raw dataset version)
4. **Given** a raw dataset with version 1.0.5 and a derivative processed from version 1.0.3, **When** outdatedness calculation runs for studies+derivatives.tsv, **Then** the derivative's outdatedness column shows the commit count between 1.0.3 and 1.0.5 in the raw dataset
...
**Independent Test**: Can be tested by running BIDS validator on study datasets and verifying outputs are stored under derivatives/bids-validator/ directory with version.txt, report.json, and report.txt files, with validation status reflected in studies.tsv.

**Acceptance Scenarios**:

1. **Given** a newly created or modified study dataset, **When** validation is triggered, **Then** bids-validator runs and outputs are saved to `derivatives/bids-validator/version.txt` (validator version), `derivatives/bids-validator/report.json` (machine-readable), and `derivatives/bids-validator/report.txt` (human-readable summary)
...
4. **Given** a study that has not changed since last validation (no commits in study except derivatives/bids-validator/), **When** validation runs with --when=new-commits (default), **Then** validation is skipped and "skipped (no changes)" is reported
...
- What happens when published-studies.json is out of sync with GitHub (manual additions/deletions)? Use `publish --sync` to reconcile: query GitHub for all study-* repos, add missing entries, remove deleted entries, update commit SHAs. Report changes found.
...
**Note on TSV File Writing**: All TSV output files MUST be written using Python's `csv.DictWriter` with `delimiter="\t"` for proper field quoting. Manual tab-joining (`"\t".join(fields)`) is FORBIDDEN because it does not escape field values containing double-quotes, tabs, or newlines. The `csv.DictWriter` approach correctly quotes JSON fields (e.g., `{"2.0":48}` becomes `"{""2.0"":48}"` in the file) and round-trips cleanly through any standard TSV/CSV reader. Do NOT use `escapechar="\\"` (causes escape-bombing on repeated read/write cycles) or `quoting=csv.QUOTE_NONE` (fails on fields containing the delimiter).
...
- **FR-003d**: System MUST consistently name sourcedata subdirectories as `sourcedata/{dataset_id}/` for ALL studies, including single-source raw datasets. The pattern `sourcedata/raw/` MUST NOT be used. This ensures consistent naming between single-source and multi-source studies (e.g., `sourcedata/ds000001/` not `sourcedata/raw/`).
- **FR-003e**: System MUST name derivative directories using sanitized `{tool_name}-{version}` pattern. When tool_name is unknown or "Custom code", use `custom-{dataset_id}` pattern instead (e.g., `derivatives/custom-ds006191/` not `derivatives/Custom code-unknown/`).
- **FR-003f**: System MUST sanitize all directory and submodule names by replacing sequences of characters that are NOT alphanumeric, hyphen (`-`), underscore (`_`), dot (`.`), or plus (`+`) with a single `+` character. This primarily removes spaces and other special characters from tool names and versions. Examples: `xcp_d-0.10.6` stays unchanged; `Custom code-unknown` becomes `Custom+code-unknown`; `tool with spaces-1.0` becomes `tool+with+spaces-1.0`.
...
- **FR-006**: System MUST populate SourceDatasets field as an array of objects following BIDS specification. Each object MUST contain: (1) URL as a BIDS URI pointing to the sourcedata submodule path (e.g., `bids::sourcedata/ds000001/`), (2) DOI if available from OpenNeuro (e.g., `doi:10.18112/openneuro.ds000001.v1.0.1`), and (3) Version if available from git tags. Example: `[{"URL": "bids::sourcedata/ds000001/", "DOI": "doi:10.18112/openneuro.ds000001.v1.0.1", "Version": "1.0.1"}]`. BIDS URIs follow https://bids-specification.readthedocs.io/en/stable/common-principles.html#bids-uri
...
- **FR-009**: System MUST generate studies.tsv with study_id, name, version, raw_version, bids_version, hed_version, license, authors, author_lead_raw, author_senior_raw, source_count, source_types, derivative_count, subjects_num, sessions_num, sessions_min, sessions_max, bold_num, bold_timepoints, bold_tasks, bold_trs, bold_duration_total, t1w_num, t2w_num, bold_size, t1w_size, bold_size_max, bold_voxels, datatypes, derivative_ids, bids_valid, and extraction_version columns. Column definitions: source_count is number of sourcedata subdatasets; source_types is comma-separated set of BIDS DatasetTypes from source datasets (e.g., "raw", "derivative", "raw,derivative"); derivative_count is number of derivative subdatasets; authors are from git shortlog of study dataset; author_lead_raw and author_senior_raw are first/last elements from Authors in raw dataset_description.json, using "n/a" if multiple sources with conflicting values, or duplicating if single author; bold_trs is distribution of TRs (repetition times) as JSON dict mapping TR in seconds to file count (e.g., '{"1.0": 500, "2.0": 100}' means 500 files at 1.0s TR, 100 at 2.0s); bold_duration_total is sum of TR×volumes across all BOLD runs in seconds; bold_voxels is maximum number of voxels in any single BOLD file; extraction_version is version of metadata extraction logic (semantic versioning MAJOR.MINOR.PATCH)
- **FR-010**: System MUST generate studies+derivatives.tsv (tall format) at top level with study_id, derivative_id as lead columns, followed by tool name, version, UUID disambiguation, size statistics, execution metrics, outdatedness, and other status columns. The `+` naming convention follows BIDS issue #2273 for TSV files with compound primary keys
- **FR-011**: System MUST generate studies.json and studies+derivatives.json describing TSV column purposes following BIDS sidecar conventions
- **FR-012**: System MUST support incremental updates (process specific studies, not all at once)
- **FR-012a**: When updating specific studies in studies.tsv or studies+derivatives.tsv, System MUST preserve existing entries for studies not being updated. Metadata generation MUST merge new/updated entries with existing entries rather than replacing the entire file. This ensures incremental updates do not cause data loss for unmodified studies.
- **FR-012b**: System MUST list non-standard TSV files (those using `+` naming convention per BIDS #2273) in the `.bidsignore` file at the top level of the BIDS dataset. This includes `studies+derivatives.tsv` and `studies+derivatives.json`. See: https://github.com/bids-standard/bids-specification/issues/2273
...
- **FR-015**: System MUST run bids-validator on study datasets and store outputs under `derivatives/bids-validator/` directory with files: `version.txt` (validator version from `--version`), `report.json` (machine-readable JSON results), `report.txt` (human-readable summary). Validation SHOULD be skipped for studies with no commits since last validation (--when=new-commits default)
...
- **FR-017b**: System MUST provide an `--include-related` option for the discover command that, when used with `--test-filter`, automatically expands the filter to include related datasets. Supported modes: `derivatives` (forward: raw→derivatives), `sources` (backward: derivative→sources), `all` (bidirectional, iterating until transitive closure). The expansion MUST be recursive by checking if any derivative's source_datasets intersect with the current filter set. When a derivative is added to the expanded set in `all` mode, ALL of its source datasets MUST also be added to ensure organize can link all required sources. The deprecated `--include-derivatives` flag remains as a backward-compatible alias for `--include-related derivatives`.
...
- **FR-024a**: System MUST provide a `publish` command that creates remote repositories on GitHub (if they don't exist) and pushes local study repositories to the configured organization. The command MUST respect the `--no-publish` flag (default: publish enabled). Publishing MUST use PyGithub (github.Repository.create_repo()) for repository creation and `git push` for content upload. Authentication relies on GITHUB_TOKEN environment variable.
- **FR-024b**: System MUST provide a `unpublish` command with safety controls to delete remote study repositories from GitHub organization. The command MUST require explicit confirmation (interactive prompt or `--confirm` flag) before deletion. The command MUST support filtering by study ID patterns (e.g., `unpublish study-ds0000*` or `unpublish --all`) and provide a `--dry-run` mode showing what would be deleted without executing. MUST use PyGithub (github.Repository.delete()) with appropriate ownership prefix (e.g., `OpenNeuroStudies/study-ds000001`).
...
- **FR-024d**: System MUST provide a `publish --sync` mode that reconciles local published-studies.json with actual GitHub state. The sync operation MUST: (1) Query GitHub API for all repositories in the configured organization matching study-* pattern, (2) Add entries for studies found on GitHub but missing from published-studies.json (manual additions), (3) Remove entries for studies in published-studies.json but not found on GitHub (manual deletions), (4) Update last_push_commit_sha for all tracked studies by querying remote HEAD, (5) Report summary of changes (X added, Y removed, Z updated). This enables recovery from manual GitHub operations and ensures tracking file accuracy.
- **FR-024e**: System MUST support optional configuration of a GitHub organization team to grant push access to published study repositories. Configuration via `maintainers_team` field in config.yaml (e.g., `maintainers_team: "Maintainers"`). When specified, the publish command MUST add the team with "push" permission to each created repository using PyGithub (github.Repository.add_to_collaborators() or team.add_to_repos()). If not configured, no additional team permissions are granted beyond repository creator. Demo/default setup SHOULD use "Maintainers" team name.
...
- **FR-029**: System MUST populate studies+derivatives.tsv with outdatedness metric for each study-derivative pair
...
- **FR-032**: System MUST extract imaging data characteristics (bold_size, t1w_size, bold_size_max, bold_trs, bold_duration_total, bold_voxels) requiring sparse data access via datalad-fuse or fsspec
- **FR-032a**: System MUST ensure consistency between related imaging metrics: if bold_size is populated (not "n/a"), then bold_voxels MUST also be populated. This consistency requirement applies to all hierarchical stats levels (per-subject, per-dataset, per-study). If sparse access fails for a file, the entire row's imaging metrics SHOULD be marked as "n/a" rather than having partial data.
...
- **FR-039**: System MUST handle multiple study dataset creation/modification operations consistently across all commands using a batch commit pattern: (1) Perform independent operations within study subdirectories (DataLad create of study dataset, commit initial state within study), (2) Under thread lock for parent repository, perform all git submodule linking operations (adding to .gitmodules and git index) without committing to parent, (3) Execute single batch commit to parent repository after all operations complete. This applies to organize, metadata generation, validation, and any other operations that modify multiple studies. The batch commit message MUST include the count of affected studies (e.g., "Organize 5 study datasets", "Update metadata for 12 studies").
- **FR-040**: System SHOULD prefer running external commands (validation, data processing) via `datalad run` for provenance tracking. Study datasets SHOULD contain `code/run-bids-validator` script designed for `datalad run code/run-bids-validator` invocation. This enables reproducible validation with full command lineage recorded in git history.
- **FR-041**: System MUST provide a `provision` command that populates study datasets with templated content: (1) `code/run-bids-validator` script for running BIDS validation via datalad run, (2) `README.md` with study dataset overview and links, (3) `.openneuro-studies/template-version` file tracking which template version was applied. The command MUST support `--force` to re-provision, `--dry-run` to preview changes, and `--when` to control when provisioning runs.
- **FR-041a**: System MUST track template version in each provisioned study via `.openneuro-studies/template-version` file. The `.openneuro-studies/` directory within each study allows for future extensibility. The `provision` command SHOULD skip studies already at current template version unless `--force` is specified. This enables efficient incremental provisioning and future template upgrades.
- **FR-042**: System MUST support hierarchical stats extraction at multiple levels: per-subject (within each source/derivative dataset), per-dataset (aggregated from subjects), per-study (aggregated from datasets), and across studies (aggregated in studies.tsv and studies+derivatives.tsv). Stats are extracted at the lowest level and consolidated upward. The extraction/aggregation framework MUST be implemented in `bids_studies` (generic, reusable BIDS library) with `openneuro_studies` providing only CLI/orchestration. The `bids_studies` package MUST NOT import from `openneuro_studies`; any shared functionality needed by both MUST reside in `bids_studies`. Sourcedata and derivatives use parallel but separate extraction modules with shared aggregation utilities (sum, max, set-union, dict-merge). **Status**: Sourcedata hierarchy implemented; derivative hierarchy partially implemented (see doc/designs/20260501-derivatives-hierarchical-aggregation.md).
- **FR-042a**: System MUST generate `sourcedata+subjects.tsv` within each study's sourcedata/ directory containing per-subject statistics. For multi-session datasets, use `sourcedata+subjects+sessions.tsv` with one row per (source_id, subject_id, session_id) combination. Files use `+` to join entity names per BIDS issue #2273.
- **FR-042b**: System MUST generate `sourcedata.tsv` within each study's sourcedata/ directory containing per-source-dataset aggregated statistics. Columns include subjects_num, sessions_num, sessions_min, sessions_max, bold_num, t1w_num, t2w_num, bold_size, t1w_size, bold_size_max, bold_trs, bold_duration_total, bold_voxels, bold_timepoints, datatypes. The bold_trs column aggregates TR distributions by merging dicts and summing counts across subjects.
- **FR-042c**: System MUST generate corresponding JSON sidecar files (sourcedata+subjects.json, sourcedata.json, derivative+subjects.json, derivatives.json) describing column purposes and units following BIDS conventions.
- **FR-042d**: System MUST store hierarchical stats files within study datasets but outside submodules (e.g., `study-ds000001/sourcedata/sourcedata+subjects.tsv` not `study-ds000001/sourcedata/ds000001/...`). This ensures original datasets remain unmodified while stats are version-controlled with the study.
- **FR-042e**: System MUST generate `derivative+subjects.tsv` (or `derivative+subjects+sessions.tsv` for multi-session) within each study's derivatives/ directory containing per-subject derivative statistics. Lead columns: derivative_id, subject_id, session_id. Stats columns: output_num, output_size, nifti_num, nifti_size, html_num, uptodate. The `uptodate` column MUST be computed per-subject by comparing whether source files for that subject changed in the raw dataset after the derivative's processed_raw_version commit (determined from git history). Derivative subdatasets MUST be installed (via `datalad install`) before extraction.
- **FR-042f**: System MUST generate `derivatives.tsv` within each study's derivatives/ directory containing per-derivative aggregated statistics plus derivative identity metadata. One row per derivative. Columns: derivative_id, tool_name, tool_version, datalad_uuid, url, size_total, size_annexed, file_count, processed_raw_version, current_raw_version, uptodate, outdatedness, tasks_processed, tasks_missing, anat_processed, func_processed, processing_complete, template_spaces, transform_spaces, descriptions, subjects_num, sessions_num, output_num, output_size, nifti_num, nifti_size, html_num. This file MUST contain all information needed to produce the top-level `studies+derivatives.tsv` without requiring access to the actual derivative subdatasets.
- **FR-042g**: System MUST aggregate stats from `sourcedata.tsv` to populate corresponding columns in studies.tsv. Aggregation uses sum for counts and totals, max for spatial dimensions (bold_voxels), and dict merge with count summation for distributions (bold_trs). No weighted means are computed; TR distributions preserve heterogeneity across acquisitions.
- **FR-042h**: System MUST generate the top-level `studies+derivatives.tsv` by reading `derivatives.tsv` from each study's derivatives/ directory and prefixing with study_id. No access to actual derivative subdatasets should be required for this aggregation step — all needed metadata is already captured in derivatives.tsv.
- **FR-042i**: All derivative metadata extraction logic (file listing, size computation from git-annex, subject counting, version tracking, completeness analysis) MUST reside in `bids_studies`. The `openneuro_studies` package provides only the CLI commands, Snakemake integration, and subdataset install/drop orchestration that calls into `bids_studies` extraction functions.
...
- **Source Dataset**: A raw BIDS dataset from OpenNeuroDatasets, openfmri, or other configured sources. Linked as git submodule under sourcedata/{dataset_id}/ (consistent naming per FR-003d). Key attributes: dataset ID, URL, commit SHA, BIDS version, license, authors.
...
- **Metadata Index**: TSV files with JSON sidecars providing tabular overviews. studies.tsv (wide format) lists studies with derivative_ids column. studies+derivatives.tsv (tall format, named per BIDS #2273) has one row per study-derivative pair with study_id and derivative_id as lead columns, enabling detailed derivative tracking. Key attributes: column names, data types, descriptions, relationships. Dashboards can join or compose wide versions as needed.
...
- User has admin/owner permissions in the target GitHub organization to create and delete repositories via PyGithub API
...
- Q: Should derivative hierarchical extraction use a single generic framework or parallel modules? → A: Parallel modules with shared aggregation utilities (same pattern as sourcedata but separate code paths)
- Q: Should per-subject uptodate tracking for derivatives live in bids_studies? → A: Yes, bids_studies computes per-subject outdatedness by checking if raw source files for that subject changed since processed_raw_version commit
- Q: How should bids_studies handle error logging without importing openneuro_studies? → A: Move needed error logging functionality into bids_studies. bids_studies MUST NOT import from openneuro_studies; any shared functionality resides in bids_studies.
- Q: Should openneuro_studies/metadata/derivative_extractor.py (1026 lines) migrate to bids_studies? → A: Yes, migrate entirely. All derivative metadata extraction moves to bids_studies. SparseDataset gets enhanced with git-annex size queries. openneuro_studies only does CLI/orchestration.
- Q: What columns belong in derivatives.tsv (per-study aggregate)? → A: All current studies+derivatives.tsv columns except study_id. derivatives.tsv contains both identity metadata (tool_name, tool_version, datalad_uuid, url) and aggregated stats from derivative+subjects.tsv. The top-level studies+derivatives.tsv is produced by reading derivatives.tsv from each study and prefixing with study_id — no derivative subdataset access needed at that stage.

### Session 2026-05-12

- Q: Should TSV files use manual tab-joining or csv.DictWriter for output? → A: All TSV output MUST use `csv.DictWriter(delimiter="\t")`. Manual `"\t".join()` is forbidden because it fails to quote fields containing double-quotes (common in JSON columns like bold_trs, descriptions), tabs, or newlines. The csv module's default QUOTE_MINIMAL quoting correctly handles JSON values (e.g., `{"2.0":48}` → `"{""2.0"":48}"`) and round-trips cleanly through csv.DictReader, pandas, R, and GitHub's TSV preview. Using `escapechar="\\"` with QUOTE_NONE causes escape-bombing (exponential backslash growth on repeated read/write cycles) and is also forbidden. This applies to FR-009, FR-010, FR-042a through FR-042h, and all other TSV generation.
