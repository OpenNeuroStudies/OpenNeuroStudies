...
- **FR-042f**: System MUST aggregate stats from `sourcedata.tsv` and `derivatives+datasets.tsv` to populate corresponding columns in studies.tsv. Aggregation uses sum for counts and totals, weighted mean for averages (weighted by bold_num for duration, by duration for voxels).
