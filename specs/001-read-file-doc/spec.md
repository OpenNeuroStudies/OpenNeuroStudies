...
- **Study Dataset**: A BIDS study folder (study-{id}) containing sourcedata/ and derivatives/ with generated metadata following BIDS 1.10.1 study specification. Key attributes: study ID, title, authors, BIDS version, source datasets, derivative datasets.
...
- **Derivative Dataset**: A processed dataset from OpenNeuroDerivatives or OpenNeuro derivatives. Linked as git submodule under derivatives/{toolname-version}/. Key attributes: tool name, version, DataLad UUID, size statistics, execution metrics, source datasets.
...
- **Metadata Index**: TSV files (studies.tsv, derivatives.tsv) with JSON sidecars providing tabular overviews. Key attributes: column names, data types, descriptions, relationships.
...
- Hosting individual study datasets as separate GitHub repositories (future refactoring per design doc TODO)
