...
- **FR-017b**: System MUST provide an `--include-derivatives` option for the discover command that, when used with `--test-filter`, automatically expands the filter to include derivatives of filtered datasets AND all source datasets of those derivatives. The expansion MUST be recursive (including derivatives of derivatives, and sources of sources) by checking if any derivative's source_datasets intersect with the current filter set. When a derivative is added to the expanded set, ALL of its source datasets MUST also be added to ensure organize can link all required sources. This enables testing with raw datasets while automatically discovering their complete dependency graphs (both downstream derivatives and upstream sources required by multi-source derivatives).
